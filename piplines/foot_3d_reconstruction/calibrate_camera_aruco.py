...
@ separator
def find_corners_aruco(dictionary_aruco, dir_input, suffix_input=".jpg", debug=False, dir_debug=None):
...
def calibrate_aruco(board_aruco, size_img, df_corners):
    ...
    [error_rms, K, dist, rvecs, tvecs, stdDeviationsIntrinsics, stdDeviationsExtrinsics, error] = aruco.calibrateCameraArucoExtended(
        corners=corners,
        ids=ids,
        counter=counter,
        board=board_aruco,
    ...
    ic(error.shape, np.max(error), np.argmax(error), df_corners.index[np.argmax(error)])
    ...
    df_trajectory.loc[index_valid, "error"] = error.tolist()
...
def evaluatle(board_aruco, dir_input, sr_camera_params: pd.Series, df_trajectory: pd.DataFrame, df_corners: pd.DataFrame, debug: int=True, dir_debug: Path or str=None):
    ...
    for name_img in df_trajectory.index:
        ...
        if df_corners.loc[name_img, "ids"] is np.nan:
            pass
        else:
            cam.set_rtvec(
    ...
    if debug:
        plt.savefig(dir_debug.parent / "calibrate_evaluatle.png", dpi = 600)
    return
