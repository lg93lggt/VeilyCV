...
@ separator
def reconstructe_as_voxel_grid(dir_input, sr_camera_params, df_trajectory, prop=0.8, z_constraint=np.array([-np.inf, np.inf]), carve_outside=True, estimate_box=False, debug=False, dir_debug=None):
...
def applay_z_colormap(mesh, resolution_z=10):
    verts = np.asarray(mesh.vertices)
    zmax = verts[:, 2].max()
    zmin = verts[:, 2].min()
    dz   = (zmax - zmin) / resolution_z
    np.sort(verts[:, 2])
    colors_table = cm.jet(range(255))[:, :3]
...
def main(dir_fit_root, dir_calib_root, size_img=None, prop=0.8, z_constraint=np.array([-0.05, np.inf]), estimate_box=False, debug=False):
    ...
    mesh = mesh.filter_smooth_simple(number_of_iterations=4)
    mesh = applay_z_colormap(mesh)

    dir_result.mkdir(parents=True, exist_ok=True)
    pth_mesh = Path(dir_result, "mesh_final.obj")
    o3d.io.write_triangle_mesh(str(pth_mesh), mesh, write_vertex_colors=True, write_vertex_normals=True, print_progress=True)
    ic(pth_mesh)
