...
def nlmeans(img, sigma, kernel_size=[21, 21]):
    ...
    # is 21 x 21
    largestride = kernel_size[0]
    smallstride = kernel_size[1]
    ...
    for y in range(largestride, h-largestride):
        for x in range(smallstride, w-smallstride):
            ...
            for yp in range(startY, endY):
                for xp in range(startX, endX):
                    ...
                    window1 = img[y-smallstride:y+smallstride, x-smallstride:x+smallstride].copy()
                    window2 = img[yp-smallstride:yp+smallstride, xp-smallstride:xp+smallstride].copy()
                    ...
                    Z += weight
                    current = current + weight*img[yp,xp]
